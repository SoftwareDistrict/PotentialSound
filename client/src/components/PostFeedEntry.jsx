...
function PostFeedEntry(props) {
    const { posterName, message, profilePic } = props.post
    const tags = ['r&b', 'rap', 'soul']
    return (
        ...
        <div>
            <Link style={{ color: '#ff8c00' }} to={`/fullMessage/${posterName}`}>
                <div id='profile' style={{ border: '2px solid black', width: '400px', height: '100px', margin: '0 auto', backgroundColor: '#3F3D3D', position: "relative" }}>
                    ...
                    <div style={{ position: "absolute", top: '20px', backgroundColor: '#ffedd8', resize: 'both', overflow: 'auto', width: '300px', height: '60px', textAlign: 'center', left: '100px', fontSize: '12px' }}>{message}</div>
